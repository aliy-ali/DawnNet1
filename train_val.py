...
class Trainer:
    def __init__(self, args, device):
        ...
        self.test_loaders = {"val": self.val_loader,"intest":self.intest_loader, "test": self.target_loader,
                             "genval":self.genval_loader }
    ...
    def do_training(self):
        ...
        self.results = {"val": torch.zeros(self.args.epochs),"intest":torch.zeros(self.args.epochs), "test": torch.zeros(self.args.epochs)
                        , "genval": torch.zeros(self.args.epochs)}

        for self.current_epoch in range(self.args.epochs):
        # for self.current_epoch in range(0, 200):
            start_time = time.time()
            self._do_epoch(self.current_epoch)
            if len(self.scheduler_scatter)>0:
                self.logger.new_epoch(self.scheduler_scatter[0].get_last_lr())
            else:
                self.logger.current_epoch+=1
            for scl in self.scheduler_scatter:##########################################每次epoch后，都要使用scheduler。
                scl.step()
            end_time = time.time()
            ...
            if (self.current_epoch+1) %10==0 :
                ...
                intest_res = self.results["intest"]
                genval_res = self.results["genval"]
                idx_best = (val_res + genval_res).argmax()
                line = "end %g, 50Best val %g,Best genval %g,avg_val_genval %g, corresponding intest %g, corresponding test %g, best val epoch: %g - best test: %g" % (
                    self.current_epoch, val_res.max(),genval_res.max(),(val_res[idx_best] + genval_res[idx_best])/2.0 , intest_res[idx_best], test_res[idx_best], idx_best, test_res.max())
                print(line)
                with open(self.base_result_path + "50_epoch_best.txt", "a") as f:
                    f.write(line + "\n")
        ...
        line = "Best val %g, corresponding intest %g, corresponding test %g, best val epoch: %g - best test: %g " % (
            val_res.max(), intest_res[idx_best], test_res[idx_best], idx_best, test_res.max())
        print(line)
...
def main():
    # for i in [ 0,4,5 , 5 ,6,7,8,9]:  # 1,456
    for i in [  6,7,8,9,0,1 ]:  # 0126num_f=5,       , 1,2,3
